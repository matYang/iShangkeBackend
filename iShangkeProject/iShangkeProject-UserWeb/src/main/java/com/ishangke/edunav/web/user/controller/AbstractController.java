...
public class AbstractController {
    ...
    protected final String getUrl(HttpServletRequest req) {
        return String.valueOf(req.getAttribute(WebConstants.REQUEST_URL));
    }
    
    protected final SessionBo getSession(HttpServletRequest req){
        try {
    ...
    protected final void openSession(SessionBo sessionBo, boolean remember, HttpServletRequest req, HttpServletResponse resp) {
        try {
    ...
    //setting the cookie
    protected final void closeSession(HttpServletRequest req, HttpServletResponse resp) {
        Cookie[] cookies = req.getCookies();
        if (cookies != null) {
            for (int i = 0; i < cookies.length; i++) {
                if (SessionSetting.COOKIE_KEY.equals(cookies[i].getName())) {
