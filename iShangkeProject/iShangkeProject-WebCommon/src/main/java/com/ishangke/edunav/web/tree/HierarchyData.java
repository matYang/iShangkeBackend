...
public class HierarchyData {
    ...
    public void addChild(HierarchyData pendingChild) throws HierarchyException{
        ...
        if ((curValLength + 2) != targetValLength) {
            throw new HierarchyException("Value length not matched, cur length: " + curValLength + " target length: " + targetValLength);
        }
        if (!this.value.equals(pendingChild.value.substring(0, curValLength))) {
            throw new HierarchyException("Prefix not match, cur value: " + this.value + " target prefix: " + pendingChild.value.substring(0, curValLength));
        }
        ...
        //fill in null as place holders, fast, and if error occurs, null pointer exception tells us asap
