...
public class AddressServiceImpl implements AddressService.Iface{
	...
	public AddressBo createAddress(AddressBo addressBo, UserBo userBo,
			PartnerBo partnerBo) throws BusinessExceptionBo, TException {
		try{
	...
	@Override
	public List<AddressBo> query(AddressBo addressBo, UserBo userBo,
		...
		try{
			...
			LOGGER.info(e.getMessage(), e);
			BusinessExceptionBo exception = new BusinessExceptionBo();
            exception.setErrorCode(ManagerErrorCode.ADDRESS_QUERY_ERROR);
            exception.setMessageKey(ManagerErrorCode.ADDRESS_QUERY_ERROR_KEY);
            throw exception;
		}
	}
