...
public class PurposeCourseManagerImpl implements PurposeCourseManager {
    ...
    public PurposeCourseBo createPurpose(PurposeCourseBo purposeCourseBo, UserBo userBo) {
        ...
        /*
         * if(authManager.isAdmin(userBo.getId()) ||
         * authManager.isSystemAdmin(userBo.getId())){
         * LOGGER.warn(String.format(
         * "[PurposeCourseManagerImpl]system admin || admin [%s] call createPurposeCourse at "
         * +new Date(), userBo.getName())); }else{ throw new
         * ManagerException("对不起，您无权执行该请求"); }
         */
