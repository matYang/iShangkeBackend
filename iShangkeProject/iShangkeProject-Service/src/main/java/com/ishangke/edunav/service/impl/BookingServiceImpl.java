...
public class BookingServiceImpl implements BookingService.Iface {
    ...
    @Override
    public String changeBookingStatusToPayed(int orderId, String trade_no) throws BusinessExceptionBo, TException {
        try {
            return bookingManager.changeBookingStatusToPayed(orderId, trade_no);
        } catch (ManagerException e) {
            LOGGER.info(e.getMessage(), e);
            BusinessExceptionBo exception = new BusinessExceptionBo();
            exception.setMessage(e.getMessage());
