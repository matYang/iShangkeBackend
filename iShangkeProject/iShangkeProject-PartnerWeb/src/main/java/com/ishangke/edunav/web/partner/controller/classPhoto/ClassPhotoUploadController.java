package com.ishangke.edunav.web.partner.controller.classPhoto;
...
@Controller
@RequestMapping("/api/v2/classPhoto/Upload")
public class ClassPhotoUploadController extends AbstractController {
    ...
    String uploadFile(@RequestParam("file") MultipartFile file, HttpServletRequest req) {
        if (!file.isEmpty()) {
            try {
                ...
                path = AliyunMain.uploadImg(partnerId, serverFile, file.getName(), Config.AliyunClassroomImgBucket);
                return path;
